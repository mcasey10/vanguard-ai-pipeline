...
- *Newly Self-Directed:* Needs the confirmation screen to feel like a safety net, not another decision. Clear language that the transaction can be reviewed (even if not reversed) after submission supports this. The tax professional advisory notice on the confirmation screen carries disproportionate emotional weight for this segment. It functions not only as a compliance disclosure but as active reassurance that the user is not making an irreversible error without professional oversight. The notice must be worded to normalize the decision rather than amplify uncertainty — it should convey that consultation is available, not that it is urgently needed.
